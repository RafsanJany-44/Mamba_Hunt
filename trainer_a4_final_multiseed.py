"""Train the final PURE and UBFC A4 candidates across seeds 101-104."""
...
from settings import DEVICE, FS, LEARNING_RATE, OUTPUT_ROOT, Experiment
...
FINAL_CONFIGS = {
    "PURE": ("L4_C100_H100", "L4_CONCENTRATION_HARMONIC", 1.0, 1.0),
    "UBFC": ("L3_C050", "L3_CONCENTRATION", 0.5, 0.0),
}
...
def model_directory(name: str) -> Path:
    return OUTPUT_ROOT / "models" / "a4_final_multiseed" / name
...
def train_one(
    ...
    name = f"{source_name}_A4_{experiment_code}_SEED{seed}"
    ...
    if source_name == "PURE":
        dataset_pure_a4.SEED = seed
        training_loader, validation_loader = dataset_pure_a4.create_pure_a4_loaders(
            source
        )
    elif source_name == "UBFC":
        dataset_ubfc_a4.SEED = seed
        training_loader, validation_loader = dataset_ubfc_a4.create_ubfc_a4_loaders(
            source
        )
    else:
...
def train_source(source_name: str) -> None:
    ...
    experiment_code, variant_code, concentration_weight, harmonic_weight = (
        FINAL_CONFIGS[source_name]
    )
    ...
    for job_number, seed in enumerate(jobs, start=1):
        name = f"{source_name}_A4_{experiment_code}_SEED{seed}"
        print("#" * 88)
        print(
            f"{source_name} A4 FINAL MULTI-SEED — "
            f"SEED {job_number}/{len(jobs)}: {seed}"
        )
        print("#" * 88)
        try:
            path = train_one(
    ...
    suite_directory = OUTPUT_ROOT / "models" / "a4_final_multiseed"
    suite_directory.mkdir(parents=True, exist_ok=True)
    suite_summary = {
        "source_dataset": source_name,
        "augmentation": "A4",
        "experiment_code": experiment_code,
        "loss_variant": variant_code,
        "pearson_weight": 0.2,
        "ce_weight": build_loss_variants(weights)[variant_code].ce_weight,
    ...
    summary_path = suite_directory / f"{source_name}_A4_final_multiseed_summary.json"
    atomic_json(summary_path, suite_summary)

    print("=" * 88)
    print(f"{source_name} A4 FINAL MULTI-SEED TRAINING FINISHED")
    print("=" * 88)
