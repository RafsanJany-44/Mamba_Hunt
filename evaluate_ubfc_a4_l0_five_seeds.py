"""Evaluate UBFC A4 L0 seeds 100-104 on six datasets and three protocols."""
...
def checkpoint_path(seed):
    ...
    return (
        MAMBA_HUNT_ROOT / "results" / "models" / "ubfc_a4_pilot" / name
...
def main():
    ...
    for number, seed in enumerate(SEEDS, start=1):
        ...
        checkpoint = checkpoint_path(seed)
        print("=" * 98)
        print(f"UBFC A4 L0 EVALUATION — SEED {number}/{len(SEEDS)}: {seed}")
        ...
        for target in DATASETS:
            ...
            run = core.EvaluationRun(
                ...
                split_end=end,
                description=(
                    f"UBFC A4 L0 five-seed experiment; seed {seed}, evaluated "
    ...
    summary_path = (
        core.OUTPUT_ROOT / "all_results_summary_UBFC_A4_L0_5x6x3.csv"
    )
    core.write_csv(summary_path, all_summaries)
    print("=" * 98)
    print("UBFC A4 L0 FIVE-SEED — 5 x 6 x 3 EVALUATION COMPLETED")
