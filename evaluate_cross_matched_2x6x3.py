"""Evaluate the two official-matched local checkpoints on six datasets.

The source dataset uses its held-out 0.8-1.0 validation partition. Every
external dataset uses its complete 0.0-1.0 manifest. Previous evaluation
...
core.OUTPUT_ROOT = (
    MAMBA_HUNT_ROOT / "results" / "evaluation_protocols_cross_matched"
)
...
CHECKPOINTS = {
    "PURE_CROSS_MATCHED": (
        ...
        / "models"
        / "PURE_CROSS_MATCHED"
        / "PURE_CROSS_MATCHED_RhythmMamba_Best.pth",
        "PURE",
    ),
    "UBFC_CROSS_MATCHED": (
        MAMBA_HUNT_ROOT
        ...
        / "models"
        / "UBFC_CROSS_MATCHED"
        / "UBFC_CROSS_MATCHED_RhythmMamba_Best.pth",
        "UBFC",
    ),
}
...
def main() -> None:
    ...
    for checkpoint_name in CHECKPOINTS_TO_EVALUATE:
        ...
        for target_dataset in DATASETS_TO_EVALUATE:
            ...
            begin, end = split_for(source_dataset, target_dataset)
            split_description = (
                "held-out 0.8-1.0 source validation split"
                if source_dataset == target_dataset
            ...
            run = core.EvaluationRun(
                ...
                split_end=end,
                description=(
                    f"Official-matched local {source_dataset} checkpoint "
    ...
    core.write_csv(
        core.OUTPUT_ROOT / "all_results_summary_cross_matched_2x6x3.csv",
        all_summaries,
    )
    print("=" * 84)
    print("CROSS-MATCHED 2 x 6 x 3 EVALUATION COMPLETED")
