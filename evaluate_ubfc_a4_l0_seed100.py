"""Evaluate UBFC_A4_L0_SEED100 on six datasets and three protocols."""
...
CHECKPOINT = (
    MAMBA_HUNT_ROOT / "results" / "models" / "ubfc_a4_pilot" / MODEL_NAME
    / f"{MODEL_NAME}_RhythmMamba_Best.pth"
)
core.OUTPUT_ROOT = (
    MAMBA_HUNT_ROOT / "results" / "evaluation_protocols_ubfc_a4_pilot"
)
...
def split_for(target):
    # UBFC is the source dataset, so report only its untouched 0.8-1.0 split.
...
def main():
    ...
    print("=" * 98)
    print(f"A4 PILOT EVALUATION: {MODEL_NAME}")
    ...
    for target in DATASETS:
        ...
        run = core.EvaluationRun(
            ...
            split_end=end,
            description=(
                f"UBFC A4 L0 seed-100 pilot evaluated on {target}: "
                f"{split_description}"
            ),
        )
    ...
    summary_path = core.OUTPUT_ROOT / "all_results_summary_UBFC_A4_L0_SEED100_1x6x3.csv"
    core.write_csv(summary_path, all_summaries)
    del model
    torch.cuda.empty_cache()
    print("=" * 98)
    print("UBFC A4 L0 SEED100 — 1 x 6 x 3 EVALUATION COMPLETED")
