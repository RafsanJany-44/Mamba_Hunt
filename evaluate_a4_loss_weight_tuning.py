"""Evaluate A4 L3/L4 weight-tuning checkpoints on 6 datasets x 3 protocols."""
...
core.OUTPUT_ROOT = (
    MAMBA_HUNT_ROOT / "results" / "evaluation_protocols_a4_loss_weight_tuning"
)
...
def checkpoint_name(source_dataset: str, tuning_code: str) -> str:
    return f"{source_dataset}_A4_{tuning_code}"


def checkpoint_path(source_dataset: str, tuning_code: str) -> Path:
    name = checkpoint_name(source_dataset, tuning_code)
    return (
        MAMBA_HUNT_ROOT
        / "results"
        / "models"
        / "a4_loss_weight_tuning"
        / name
...
def validate(source_dataset: str) -> None:
    ...
    for tuning_code in TUNING_CODES_TO_EVALUATE:
        checkpoint = checkpoint_path(source_dataset, tuning_code)
...
def completed_summary(run: core.EvaluationRun, protocol) -> dict | None:
    ...
    if summary.get("protocol") != protocol.name:
        return None
    if summary.get("completion_status") != "PASSED":
        return None
    if Path(summary.get("checkpoint", "")).resolve() != run.checkpoint.resolve():
        return None
...
def evaluate_source(source_dataset: str) -> None:
    ...
    for job_number, tuning_code in enumerate(TUNING_CODES_TO_EVALUATE, start=1):
        name = checkpoint_name(source_dataset, tuning_code)
        checkpoint = checkpoint_path(source_dataset, tuning_code)
        print("=" * 96)
        print(
            f"{source_dataset} A4 LOSS-WEIGHT EVALUATION — "
            f"CHECKPOINT {job_number}/{len(TUNING_CODES_TO_EVALUATE)}"
        )
        ...
        for target_dataset in DATASETS_TO_EVALUATE:
            ...
            run = core.EvaluationRun(
                ...
                split_end=end,
                description=(
                    f"A4 {tuning_code}, local {source_dataset} checkpoint evaluated "
                    f"on {target_dataset}: {split_description}"
                ),
            )
    ...
    summary_path = (
        core.OUTPUT_ROOT
        / f"all_results_summary_a4_loss_weight_tuning_{source_name}_8x6x3.csv"
    )
    core.write_csv(summary_path, all_summaries)
    print("=" * 96)
    print(f"{source_dataset} A4 LOSS-WEIGHT TUNING 8 x 6 x 3 COMPLETED")
