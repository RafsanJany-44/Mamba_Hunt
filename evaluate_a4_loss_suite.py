"""Evaluate A4 + L1-L4 checkpoints on six datasets and three protocols."""
...
core.OUTPUT_ROOT = (
    MAMBA_HUNT_ROOT / "results" / "evaluation_protocols_a4_loss_suite"
)
...
def checkpoint_name(source_dataset: str, variant: str) -> str:
    return f"{source_dataset}_A4_{variant}"


def checkpoint_path(source_dataset: str, variant: str) -> Path:
    name = checkpoint_name(source_dataset, variant)
    return (
        MAMBA_HUNT_ROOT
        / "results"
        / "models"
...
def validate(source_dataset: str) -> None:
    ...
    for variant in VARIANTS_TO_EVALUATE:
        checkpoint = checkpoint_path(source_dataset, variant)
...
def evaluate_source(source_dataset: str) -> None:
    ...
    for job_number, variant in enumerate(VARIANTS_TO_EVALUATE, start=1):
        name = checkpoint_name(source_dataset, variant)
        checkpoint = checkpoint_path(source_dataset, variant)
        print("=" * 96)
        print(
            f"{source_dataset} A4 LOSS-SUITE EVALUATION — "
            f"CHECKPOINT {job_number}/{len(VARIANTS_TO_EVALUATE)}"
        )
        ...
        for target_dataset in DATASETS_TO_EVALUATE:
            ...
            run = core.EvaluationRun(
                ...
                split_end=end,
                description=(
                    f"A4 {variant}, local {source_dataset} checkpoint evaluated "
    ...
    summary_path = (
        core.OUTPUT_ROOT
        / f"all_results_summary_a4_loss_suite_{source_name}_4x6x3.csv"
    )
    core.write_csv(summary_path, all_summaries)
    print("=" * 96)
    print(f"{source_dataset} A4 LOSS SUITE 4 x 6 x 3 COMPLETED")
